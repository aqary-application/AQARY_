  ...
  Future<void> _loadStats() async {
    ...
    try {
      ...
      final results = await Future.wait([
        ...
        db.collection('listings').where('status', isEqualTo: 'active').count().get(),
        db.collection('providers').where('isHidden', isEqualTo: false).count().get(),
        db.collection('users').where('isBlocked', isEqualTo: true).count().get(),
      ]);
      if (mounted) {
        setState(() {
                ...
                : Column(children: [
                    Row(children: [
                      Expanded(child: _StatCard(
                          ...
                          icon: Icons.people_outline, color: AppTheme.primary)),
                      const SizedBox(width: 12),
                      Expanded(child: _StatCard(
                          label: 'Active Listings', value: '$_totalListings',
                          icon: Icons.location_on_outlined, color: const Color(0xFF10B981))),
                    ]),
                    const SizedBox(height: 12),
                    Row(children: [
                      Expanded(child: _StatCard(
                          label: 'Providers', value: '$_totalProviders',
                          icon: Icons.engineering_outlined, color: const Color(0xFFF59E0B))),
                      const SizedBox(width: 12),
                      Expanded(child: _StatCard(
                          label: 'Blocked', value: '$_blockedUsers',
